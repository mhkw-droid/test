...
## Projekt komplett neu aufbauen (sauberer Neustart)

Wenn du wirklich einmal komplett „frisch“ starten willst:

1. `make post-pull`
2. `make repair-compose` (nur wenn Schritt 1 Fehler zeigt)
3. `make clean-rebuild`

Das stoppt alte Container, baut ohne Cache neu und startet den Stack wieder.

## Altes beim Merge komplett überschreiben

### Sicherste Variante (Repository exakt wie Remote-Branch)

> Achtung: lokale Änderungen gehen dabei verloren.

- `git status` (muss sauber sein)
- `make overwrite-master`  
  (oder `make overwrite-main`, je nach Default-Branch)

Das entspricht technisch einem `git reset --hard origin/<branch>` + `git clean -fd`.

### Wenn du in einem laufenden Merge bist

- Merge abbrechen: `git merge --abort`
- Dann harte Übernahme vom Remote: `make overwrite-master` (oder `make overwrite-main`)

### Konflikte künftig minimieren

- Vor jedem Pull: `make post-pull`
- Nach jedem Pull: `make post-pull`
- Bei Compose-Fehlern sofort: `make repair-compose`
